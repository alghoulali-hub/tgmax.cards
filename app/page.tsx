...
export default async function Home() {
  ...
  try {
    ...
    const { data } = await supabase.from("cards").select("id,title,card_code,image_key,price_cents,categories(name)").eq("status", "active").order("updated_at", { ascending: false });
