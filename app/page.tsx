...
export default async function Home() {
  ...
  try {
    const query = await env.DB.prepare(`SELECT cards.id, cards.title, cards.card_code, cards.price_cents,
      ...
      WHERE cards.status = 'active' ORDER BY cards.updated_at DESC`).all<{
        id: number; title: string; card_code: string; price_cents: number; series: string; accent: string;
      }>();
